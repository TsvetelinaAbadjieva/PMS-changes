...
function redirectToProject() {
  ...
  callAjax('POST', url, headerConfig, reqObj, function (data) {
    ...
    if (data.data.length == 0 && _document.getElementById('sectionDefault') == undefined) {
      clonePrimarySection(projectId, _document, sectionId);
      if (!_document.getElementById('sectionDefault')) {
...
function addStatusCollection() {
  ...
  var status = document.getElementById('changeStatus');
  ...
  callAjax('GET', url, headerConfig, null, function (data) {
    ...
    for (var i = 0; i < data.data.length; i++) {
      ...
      option.innerHTML = data.data[i].status;
      fragment.append(option);
...
document.getElementById('csvDownload').addEventListener('click', function () {
  console.log(taskDataCollection);
  var csv = convertToCSV(taskDataCollection);
  document.getElementById('csvDownload').textContent = 'download';
  this.download = projectName + "_tasks.csv";
