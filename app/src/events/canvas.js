    ...
    if (projectId > 0) {
        ...
        callAjax('POST', url, headerConfig, reqObj, function (data) {
            ...
            if (data.data.length == 0) {
                ...
                canvas.width = (4 / 2) * taskCellWidth * dateCollection.length + 2 * options.padding;
...
function drawDateCollection(options) {
    ...
    for (var i = 0; i < options.data.length; i++) {
        ...
        switch (options.type) {
            ...
            case 'date': dataTitle = {
                data: options.data[i],
                strPntX: x + cellWidth* (i+1),
                strPntY: y
            };
                dateArr.push(dataTitle);
                ctx.rect(dataTitle.strPntX , dataTitle.strPntY, dataTitle.strPntX + options.cellWidth, options.cellHeight);
                break;
        }
        ctx.fillText(options.data[i], dataTitle.strPntX + 2 * options.padding , dataTitle.strPntY + options.padding, options.cellHeight);
        //ctx.stroke();
    }
};
...
function drawTitle(options, dateCollection, taskCellWidth) {
    ...
    ctx.fillText(options.project + '   from ' + options.data[0] + '  to  ' + options.data[len - 1], options.beginPntX + 150, options.beginPntY - 20);
    ctx.restore();
...
function drawTask(options) {
    ...
    maxY = upperLeftCornerY +height / 2 +30;
};
...
function drawLegend(options) {
    ...
    //var height = canvas.height * 2 / 3 + 100;
    //ctx.fillText('Legend', canvas.width / 4, canvas.height * 2 / 3 - 70);
    var height = maxY + 100;
    ctx.fillText('Legend', canvas.width / 4, height - 70);
    ...
    for (var i = 0; i < options.series.length; i++) {
        ...
        ctx.fillRect(canvas.width / 4 - 35, height-40 , 30, 30);
        ctx.fillStyle = "#30333c";
        ctx.fillText(item.value, canvas.width / 4, height -40);
        ctx.restore();
        maxY = height;
    }
    height +=40;
