...
exports.getSectionTaskCollection = function (res, projectId, func) {
    ...
    con.query(sql, [projectId], function (err, result) {
        ...
        for (var i = 0; i < result.length; i++) {
          ...
          result[i].startDate = parseDate(result[i].startDate);
          result[i].dueDate = parseDate(result[i].dueDate);
          result[i].now = parseDate(result[i].now);
          // console.log(result[i].startDate);
          // console.log(result[i].dueDate);
          // console.log(result[i].now );
...
//get all task statuses from status table
exports.getStatusCollection = function (res, func) {

    var sql = "SELECT * " +
              "FROM project_management_system.task_status";
    var resultCollection = [];
    con.query(sql, function (err, result) {
